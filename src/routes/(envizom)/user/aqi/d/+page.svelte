<script lang="ts">
    ...
    onMount(fetchEnv);
    let currentAqi = $derived($aqi_and_units.aqi_id);
    let selectedAqi = $derived(currentAqi);
    const device = $derived(
        $devices.filter((x) => !$aqi_and_units.device_aqi_ids[x.deviceId]),
    );
    const cluster = $derived(
        $clusters.filter((x) => !$aqi_and_units.device_aqi_ids[x.clusterId]),
    );
    const aqi = $derived(
        ($env.type === "success"
...
>
    <div class="p-4">
        ...
        <br />
        Currently Applied To: {#each device as d}
    ...
    <div class="flex">
        <div class="h-13 flex mt-3 w-full">
            {#if $env.type === "success"}
                <select bind:value={selectedAqi} class="bg-transparent m-1 t">
                    ...
                    {#each aqi.details.measure_strategy.sub_indexes as subIndex}
                        ...
                        >
                            {$aqi_and_units.units[subIndex.parameter_id]
                                .parameter_label} <br />({$aqi_and_units.units[
                ...
                >
                    ...
                    >
                        {aqi.details.measure_strategy.zones.next[i - 1]
                            ?.upper ?? aqi.details.measure_strategy.zones.lower}
